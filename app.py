...
def GetData():
    ...
    x = datetime.now() - timedelta(hours = 2) #GMT is 5 hours behind
    ...
    prerictions['xdate'] = str(f"{x.year}-{x.month}-{x.day-1} {x.hour}:{x.minute}:{x.second}")
...
def chage_password():
    ...
    admin_data = pd.read_csv('admin.csv')
    if admin_data.iloc[:,1].values == username:
        admin_data['pass'] = passw
        admin_data.to_csv('admin.csv',index = False)
        return render_template('admin_login.html')
    else:
        return render_template('change_pass.html',x = 'Invalid Credentials')
